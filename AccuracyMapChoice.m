...
mmmm = [1 8 15 22 29 36 43 2 3 44 45; 7 14 21 28 35 42 49 6 5 48 47];


for i = 1:size(mmmm, 2)
    mtAll{mmmm(1, i)} = [mtAll{mmmm(1, i)} mtAll{mmmm(2, i)}]; 
    mtTrial{mmmm(1, i)} = [mtTrial{mmmm(1, i)} mtTrial{mmmm(2, i)}];
    mtAll{mmmm(2, i)} = [mtAll{mmmm(1, i)} mtAll{mmmm(2, i)}]; 
    mtTrial{mmmm(2, i)} = [mtTrial{mmmm(1, i)} mtTrial{mmmm(2, i)}];
end
...
save('.\AccuracyMap\AccuracyMapLasso200Choice', 'img');

label = {'Choice', 'Reward', 'Ramp'};
offsetLabel = {'Current', 'Previous'};


mmmm = [1:7 8 15 14 21; 43:49 36 29 42 35];
...
for j = 1:size(img, 2)
   for k = 1:1%size(img, 3)
       ...
       for i = 1:size(mmmm, 2)
          mnACC{j, k}(mmmm(1, i)) = mean([mnACC{j, k}(mmmm(1, i)) mnACC{j, k}(mmmm(2, i))]);
          mnACC{j, k}(mmmm(2, i)) = mean([mnACC{j, k}(mmmm(1, i)) mnACC{j, k}(mmmm(2, i))]);
       end
   end
end
