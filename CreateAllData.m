...
function [data, TS] = CreateAllData(folderPath, timeRange, locationFileName)
...
if(nargin < 3 )
    locationFileName = 'vtm1.pvd';
end

locationFile = [folderPath, locationFileName];
...
AllTrials = find(times >= TS(1) & times <= TS(2));
times = times(AllTrials);
...
for i=1:numel(times)-1
    trialNum(indMin(i):indMin(i+1)-1) = AllTrials(i);
end  

df = diff(indMin);
%badTrials = find((df / median(df)) > 1.4 | (df / median(df)) < 0.3333);

badTrials = find((df / median(df)) > 1.4);
badTrials = AllTrials(badTrials);
...
AllDataBad = AllData(:, ismember( AllData(trialIndex, :), badTrials) );
AllDataBad = AllDataBad(:, AllDataBad(trialIndex, :) ~= 0);
AllDataBad = AllDataBad(:, 1:floor(length(AllDataBad)/1000) * 1000);
AllDataBad = AllDataBad(:, AllDataBad(locIndex(1), :) ~= 0);
...
data.data = AllData;
data.badData = AllDataBad;
data.dataIndex = dataIndex;
data.trialIndex = trialIndex;
data.locIndex  = locIndex;
data.timeIndex = timeIndex;
data.trInfo = trial_events;
data.trials = unique(AllData(trialIndex, :));
data.badTrials = unique(AllDataBad(trialIndex, :));
